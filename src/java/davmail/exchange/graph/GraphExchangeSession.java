...
public class GraphExchangeSession extends ExchangeSession {
    ...
    private void preserveMimeOrigin(GraphObject graphResponse, MimeMessage mimeMessage, Map<String, String> properties) throws JSONException {
        ...
        try {
            ...
            LOGGER.warn("Unable to read sent date of appended message: " + e.getMessage());
        }
        ...
        try {
            ...
            LOGGER.warn("Unable to read header lines of appended message: " + e.getMessage());
        }
        if (headerLines != null) {
        ...
        if (headers.length() > 0) {
            graphResponse.put("messageheaders", headers.toString());
        }
        // threading headers are not among those Exchange rebuilds from the
        // transport headers, but have MAPI properties of their own
        String inReplyTo = getMessageHeader(mimeMessage, "In-Reply-To");
        if (inReplyTo != null) {
            graphResponse.put("inreplyto", inReplyTo.trim());
        }
        String references = getMessageHeader(mimeMessage, "References");
        if (references != null) {
    ...
    /**
     * Get message header value from MIME message.
     * Warn on error, do not throw exception.
     * @param mimeMessage MIME message
     * @param headerName header name
     * @return header value
     */
    protected String getMessageHeader(MimeMessage mimeMessage, String headerName) {
        String headerValue = null;
        try {
            headerValue = mimeMessage.getHeader(headerName, " ");
        } catch (MessagingException e) {
            LOGGER.warn("Unable to read header '" + headerName + "' of appended message: " + e.getMessage());
        }
        return headerValue;
    }
